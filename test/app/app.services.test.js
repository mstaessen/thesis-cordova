...
//    });
    ...
    describe('CurrencyService', function () {
        it('.getCurrencyCodes()', inject(function (CurrencyService) {
            var currencyCodes;

            runs(function () {
                console.log('Starting test');
                CurrencyService.getCurrencyCodes().then(function(data) {
                    currencyCodes = data;
                }, function(error) {
            ...
            waitsFor(function () {
                return undefined !== currencyCodes;
            }, 'fetching currency codes', 20000);
            ...
            runs(function () {
                ...
                expect(currencyCodes).toContain('BGN');
                // ... and many more
            });
        }));

        it('.convert(10, \'USD\')', inject(function (CurrencyService) {
            var amount;

            runs(function () {
                console.log('Starting test');
                CurrencyService.convert(10, 'USD').then(function(result) {
                    amount = result;
                }, function(error) {
                    console.log('ERROR: ' + error);
                });
            }, 'convert 10USD to EUR');

            waitsFor(function () {
                return undefined !== amount;
            }, 'converting 10USD to EUR', 20000);

            runs(function () {
                expect(amount).toEqual(jasmine.any(Number));
            });
        }));
    });
});